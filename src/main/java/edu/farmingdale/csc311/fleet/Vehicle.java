...
public abstract class Vehicle implements Honkable {
    ...
    protected Vehicle(String vin, String make, String model, int year, String color, int wheels, double engineSize, FuelType fuelType, double fuelCapacity) {
    ...
    public String toString() {
        String engine = fuelType.hasEngine() ? String.format("%.1fL", engineSize) : "n/a";

        return String.format(
                "%d %s %s [VIN=%s] color=%s, wheels=%d, engine=%s, fuel=%s, capacity=%.1f %s",
                year, make, model, vin, color, wheels, engine, fuelType.getLabel(), fuelCapacity, fuelType.getUnit());
    }
